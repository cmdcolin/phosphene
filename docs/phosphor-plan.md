...
- **Phase 1 — done** (commit `0c141f0`). Hue-preserving `gamutFit()` replaced
  the hard clamp in `decode`; `crtCutoff`/`crtGamma`/`crtSat` beam transfer
  added to `crt_face` (transfer → saturate → gamut-fit → bloom, bloom taps
  included). All three params default to identity, so no existing preset or
  gallery asset moved. Two opt-in presets shipped under a new `Phosphor / CRT`
  group.
- **Phase 2 — done.** `phosphorMode` (0 sRGB, 1 P22/SMPTE-C, 2 NTSC-1953 on
  ...
  spec below: the matrices are applied in _linear_ light (pow 2.2 around the 3×3
  — the matrix acts on photons, not gamma-encoded drive); naive additive
...
1. **`decode.wgsl` ends in `clamp(rgb, 0, 1)`.** A hard clamp on an out-of-gamut
   colour changes its hue — clipping only the channel that overflows rotates the
   colour toward the remaining primaries. Saturated content therefore goes
   _duller and wrong_ at exactly the brightness where a real tube goes _more
   electric_.
2. **There is no display-side black cut.** `fbBlack` exists but lives in
   `compose.wgsl`, which is the feedback _camera_ path only. Nothing lifts or
...
3. **Persistence decay is hard-coded.** `decode` uses
   `vec3f(pow(g, 1.7), g, pow(g, 2.4))`. That ratio _is_ the green-tailed trail,
...
- **`crtSat`** — saturation applied around luma, _after_ the transfer.
...
- **Consider decay vs peak-hold.** Persistence currently uses `max()`, which
  holds a hard edge. Real phosphor decays exponentially and _adds_. A mix
...
- **`phosphorMode` (P22 matrix).** A discrete select, not a slider — follow the
  ...
  e.g. `P.phosphorMode < 0.5`). Apply the 3×3 to `rgb` _after_ the YIQ→sRGB
  matrix (`decode.wgsl:171-175`) and _before_ `gamutFit`, so the fit still
  ...
  no select control yet — either add one to `SliderDef`/the controls renderer,
  or interim-ship it as a stepped slider (min 0, max 2, step 1) to avoid new UI
  work.
...
Verify with `?preset=` + `scripts/shot.mjs` (Firefox Nightly) before/after
across the preset list; with all new params at their defaults the captures must
be byte-identical to Phase 1.
...
shaders plus params. Phase 2 changes the colour of _every existing preset_,
