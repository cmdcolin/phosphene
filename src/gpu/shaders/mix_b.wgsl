...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  // 90 degrees per sample off the lattice
  let delta = P.bPhase0 + P.bPhaseLine * f32(row) + 0.5 * PI * frac;
  ...
  if (srow < VSYNC_FIRST || (srow > VSYNC_LAST && srow < 12u)) {
    ...
    b = IRE_BLACK + VIDEO_RANGE * yuvB[np].x + VIDEO_RANGE * (uf * sc.x + vf * sc.y);
  }

  // sum at the composite level; ring mod multiplies the two signals
  let a = comp[n];
  comp[n] = a + P.bGain * b + P.bRing * a * b * 0.01;
}
